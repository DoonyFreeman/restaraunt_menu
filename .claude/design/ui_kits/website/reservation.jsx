...
const { Button, Input } = window.ChaiShopperDesignSystem_69d5ad;
        ...
        return (
          <React.Fragment key={i}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, width: 64 }}>
              <div style={{ width: 30, height: 30, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 600,
                ...
                border: `1px solid ${cur || done ? 'var(--border-gold)' : 'var(--border-subtle)'}` }}>{done ? '✓' : i + 1}</div>
...
function ReservationScreen({ go }) {
  const labels = ['Точка', 'Дата', 'Гости', 'Контакты', 'Готово'];
  const [step, setStep] = React.useState(0);
  const [f, setF] = React.useState({ loc: null, date: '', time: '19:00', guests: 2, name: '', phone: '', email: '' });
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 76px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40,
      ...
      <div style={{ width: 560, maxWidth: '100%', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-overlay)', padding: 40 }}>
        ...
        {step === 1 && (
          <Field title="Дата и время">
            ...
            <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
              {times.map(t => {
                const on = t === f.time;
                return <button key={t} onClick={() => set('time', t)} style={{ padding: '9px 15px', borderRadius: 'var(--radius)', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 14,
                  background: on ? 'var(--accent)' : 'transparent', color: on ? 'var(--text-on-gold)' : 'var(--text-secondary)', border: `1px solid ${on ? 'var(--accent)' : 'var(--border-subtle)'}` }}>{t}</button>;
              })}
            </div>
          </Field>
        )}
        ...
        {step === 4 && (
          <div style={{ textAlign: 'center', padding: '12px 0 8px' }}>
            ...
            <div style={{ marginTop: 30 }}><Button variant="primary" onClick={() => go('home')}>На главную</Button></div>
          </div>
        )}

        {step < 4 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 36 }}>
            <Button variant="ghost" onClick={step === 0 ? () => go('home') : back}>{step === 0 ? 'Отмена' : '← Назад'}</Button>
